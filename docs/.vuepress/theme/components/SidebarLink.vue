<script>
    ...
    export default {
        functional: true,
        props: ['item'],
        render (createElement, {parent: {$page, $site, $route}, props: {item}}) {
            ...
            const active = item.type === 'auto'
                ...
                : selfActive
            const link = renderLink(createElement, item.path, item.title || item.path, active)
            ...
            if (item.type === 'auto') {
                return [link, renderChildren(createElement, item.children, item.basePath, $route, maxDepth)]
            } else if ((active || displayAllHeaders) && item.headers && !hashRE.test(item.path)) {
                const children = groupHeaders(item.headers)
                return [link, renderChildren(createElement, children, item.path, $route, maxDepth)]
            } else {
                return link
            }
        }
    ...
    function renderChildren (createElement, children, path, route, maxDepth, depth = 1) {
        if (!children || depth > maxDepth) return null

        return createElement('ul', {class: 'sidebar-sub-headers'}, children.map(child => {
            const active = isActive(route, path + '#' + child.slug)

            return createElement('li', {class: 'sidebar-sub-header'}, [
                renderLink(createElement, path + '#' + child.slug, child.title, active),
                renderChildren(createElement, child.children, path, route, maxDepth, depth + 1)
            ])
        }))
    }
</script>
